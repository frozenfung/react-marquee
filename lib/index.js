...
var Marquee = function (_Component) {
  ...
  _createClass(Marquee, [{
    ...
    value: function render() {
      ...
      const p = this.props.text.map(function(row) {
      ...
      const link = _react2.default.createElement(
        'button',
        { className: 'marquee-btn', style: style, onClick: this.props.marqueeBtnClick },
        this.props.btnText
      )
      ...
      if (this.state.overflowHeight < 0) {
        return _react2.default.createElement(
          'div',
          { className: 'ui-marquee ' + this.props.className, style: { overflow: 'hidden', height: '50vh' } },
          _react2.default.createElement(
            'div',
            { ref: 'text', className: 'marquee-wrap' },
            p, link
          )
        );
      } else {
    ...
    value: function _startAnimation() {
      ...
      clearTimeout(this._marqueeTimer);
      var isLeading = this.state.animatedHeight === 0;
      var timeout = isLeading ? this.props.leading : TIMEOUT;

      var animate = function animate() {
        var overflowHeight = _this2.state.overflowHeight;

        var animatedHeight = _this2.state.animatedHeight + STEP;
        var isRoundOver = animatedHeight > overflowHeight;
    ...
    value: function _measureText() {
      ...
      if (container && node) {
        ...
        var textHeight = node.offsetHeight;
        var overflowHeight = textHeight - containerHeight;

        if (overflowHeight !== this.state.overflowHeight) {
          this.setState({
            animatedHeight: - 1.2 * textHeight,
